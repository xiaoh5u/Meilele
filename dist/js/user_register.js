...
$('.registerBox').on('input', function (e) {
    ...
    if (target.id === 'username') {
        check(/^[a-zA-Z_\u4e00-\u9fa5][\w\u4e00-\u9fa5]{3,19}$/, $(target).val(), '.userName i', '.username_p', target);
    }
    if (target.id === 'password') {
        check(/(?![A-Z]+$)(?![a-z]+$)(?!\d+$)\S{6,19}$/, $(target).val(), '.userPwd i', '.password_p', target);
    ...
    if (target.id === 'confirm') {
        ...
        //密码框内有内容时进行以下验证
        if ($('#password').val()) {
            check(new RegExp('^' + $('#password').val() + '$'), $(target).val(), '.pwdConfirm i', '.confirm_p', target);
        }
    }
    ...
    if (target.id === 'input_code') {
        ...
        check(new RegExp('^' + str.toUpperCase() + '$'), $(target).val().toUpperCase(), '.code i', '.code em', target);
    }
});
